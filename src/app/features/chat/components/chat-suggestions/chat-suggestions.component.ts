    ...
    <div class="flex flex-wrap gap-2" aria-label="Suggested prompts">
      @for (prompt of prompts; track prompt.id) {
        <button type="button" (click)="choose(prompt.label)"
                class="min-h-11 rounded-full border border-hairline bg-surface px-3 py-2 text-xs font-semibold text-ink shadow-card hover:bg-sunken focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2">
          {{ prompt.label }}
        </button>
      }
    </div>
