  ...
  template: `
    <app-auth-split-card
      ...
      <div class="mx-auto flex w-full max-w-md flex-col">
        <h1 class="text-3xl font-bold leading-tight tracking-tight text-ink md:text-5xl">Create a New Password</h1>
        <p class="mt-3 text-base leading-relaxed text-ink-soft">Choose a new password to secure your Calmi account.</p>

        @if (!authService.isAuthenticated()) {
          <div class="mt-6 rounded-xl border border-danger bg-surface px-4 py-3 text-sm text-danger" role="alert" aria-live="assertive">This reset link is invalid or expired. Request a new link and try again.</div>
        }
        @if (errorMessage()) {
        ...
        <form class="mt-8 space-y-5" [formGroup]="resetForm" (ngSubmit)="submit()" novalidate>
          <div>
