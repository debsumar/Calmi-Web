    ...
    <app-auth-split-card
      ...
      <div class="mx-auto flex w-full max-w-md flex-col">
        ...
        @if (confirmationMessage()) {
          <div appAnimateOnScroll style="--index:2" class="mt-6 rounded-xl border border-brand bg-sunken px-4 py-3 text-base text-ink" role="status" aria-live="polite">{{ confirmationMessage() }}</div>
        }
        ...
        <p appAnimateOnScroll style="--index:4" class="mt-8 text-center text-sm text-ink-soft"><a routerLink="/auth/login" class="font-semibold text-ink underline underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">Back to Login</a></p>
      </div>
    </app-auth-split-card>
  `,
})
